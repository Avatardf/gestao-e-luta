      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="space-y-4">
            {pilares.map((p) => (
              <div key={p.num} className="card group flex gap-5 items-start">
                ...
                <div>
                  <h3 className="font-heading text-xl text-white tracking-widest mb-1">{p.titulo}</h3>
        ...
        <div className="mt-20 grid grid-cols-2 md:grid-cols-4 gap-px bg-navy-700">
          {[
            ...
            <div key={s.label} className="bg-navy-900 px-8 py-8 text-center">
              <div className="font-heading text-3xl md:text-4xl text-gold-500 mb-1">{s.valor}</div>
              <div className="text-gray-400 text-xs uppercase tracking-widest font-heading">{s.label}</div>
            </div>
          ))}
        </div>
      </div>
