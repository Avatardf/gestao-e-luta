  ...
  return (
    ...
    >
      <div className="flip-card-inner">
        ...
        {/* ── FRENTE ─────────────────────────────────────────────── */}
        <div className="flip-card-front bg-white dark:bg-navy-900 border border-slate-200 dark:border-navy-700 flex flex-col overflow-hidden">
          {/* Avatar */}
          <div className="w-full h-48 bg-slate-100 dark:bg-navy-800 flex items-center justify-center border-b border-slate-200 dark:border-navy-700 flex-shrink-0 relative">
            {d.foto
              ? <img src={d.foto} alt={d.nome} className="w-full h-full object-cover object-top" />
              : (
                <div className="w-20 h-20 rounded-full bg-slate-200 dark:bg-navy-700 border-2 border-gold-500/40 flex items-center justify-center">
                  <span className="font-heading text-2xl text-gold-400">{initials}</span>
                </div>
              )
            }
            {/* hint */}
            <div className="absolute bottom-2 right-3 text-slate-400 dark:text-gray-600 text-xs font-heading tracking-wide select-none">
          ...
          <div className="p-5 flex flex-col flex-1">
            ...
            <h3 className="font-heading text-lg text-slate-900 dark:text-white tracking-wide mt-0.5">{d.nome}</h3>
            {d.delegacia && <p className="text-slate-500 dark:text-gray-500 text-xs mt-0.5">{d.delegacia}</p>}

            <p className="text-slate-600 dark:text-gray-400 text-sm leading-relaxed mt-3 flex-1 overflow-hidden">{d.resumo || d.bio}</p>
        ...
        <div className="flip-card-back bg-white dark:bg-navy-900 border border-gold-500/40 flex flex-col overflow-hidden">
          ...
          <div className="bg-gold-500/10 border-b border-gold-500/30 px-5 py-4">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-full bg-slate-100 dark:bg-navy-800 border-2 border-gold-500 flex items-center justify-center flex-shrink-0">
            ...
            <div className="mt-auto pt-4 border-t border-slate-200 dark:border-navy-700 space-y-2">
              {d.whatsapp && (
                <a
                  ...
                  className="flex items-center gap-2 bg-green-500/10 border border-green-500/30 px-4 py-2.5 text-green-600 dark:text-green-400 hover:bg-green-500/20 transition-colors w-full"
                >
              ...
              {d.instagram && (
                <a
                  ...
                  rel="noopener noreferrer"
                  onClick={e => e.stopPropagation()}
                  className="flex items-center gap-2 bg-pink-500/10 border border-pink-500/30 px-4 py-2.5 text-pink-600 dark:text-pink-400 hover:bg-pink-500/20 transition-colors w-full"
                >
                  <Instagram size={15} />
...
export default function Diretores() {
  ...
  return (
    <section id="diretores" className="py-24 bg-white dark:bg-navy-950">
