...
import { Shield, RefreshCw, Trash2, LogOut, Loader2, Users, Vote, MapPin, Clock } from 'lucide-react'
...
export default function Admin() {
  ...
  async function loadData() {
    setLoading(true)
    const [{ data: v }, { data: vi }] = await Promise.all([
          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              <div className="card text-center">
                ...
                <div className="text-gray-500 text-xs uppercase tracking-widest font-heading mt-1">Votos registrados</div>
              </div>
              <div className="card text-center">
                <Users className="text-blue-400 mx-auto mb-2" size={22} />
                <div className="font-heading text-3xl text-blue-400">{visits.length}</div>
                <div className="text-gray-500 text-xs uppercase tracking-widest font-heading mt-1">Visitas ao site</div>
              </div>
              <div className="card text-center">
                <MapPin className="text-green-400 mx-auto mb-2" size={22} />
                <div className="font-heading text-3xl text-green-400">
                  {new Set(votes.map(v => v.city)).size}
                </div>
              ...
              <div className="card text-center">
                ...
                <div className="text-gray-500 text-xs uppercase tracking-widest font-heading mt-1">Meta ({META} votos)</div>
              </div>
            </div>
            ...
            <div className="card mb-8">
              ...
              <div className="h-4 bg-navy-800 overflow-hidden mb-2">
                <div
                  className={`h-full transition-all duration-700 ${pct >= 100 ? 'bg-green-500' : pct >= 60 ? 'bg-gold-500' : 'bg-blue-500'}`}
                  style={{ width: `${pct}%` }}
                />
              </div>
            ...
            <div className="flex gap-1 mb-4 border-b border-navy-700">
              {['votos', 'visitas'].map(t => (
                ...
                >
                  {t === 'votos' ? `Votos (${votes.length})` : `Visitas (${visits.length})`}
                </button>
              ))}
            </div>
                  ...
                  <tbody>
                    {votes.map((v, i) => (
                      <tr key={v.id} className="border-b border-navy-800 hover:bg-navy-900 transition-colors">
                        <td className="px-4 py-3 text-gray-600 text-xs">{totalVotos - i}</td>
            ...
            <div className="mt-12 pt-8 border-t border-navy-800">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-gray-500 text-xs mt-1">Apaga todos os votos da simulação. Visitas não são afetadas.</p>
                </div>
                <button
                  onClick={handleReset}
                  disabled={resetting}
