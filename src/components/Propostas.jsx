...
export default function Propostas() {
  return (
    <section id="propostas" className="py-24 bg-navy-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {propostas.map((p) => (
            <div key={p.id} className="card group hover:-translate-y-1 transition-all duration-300">
              ...
              <div className="flex items-start gap-4 mb-4">
                ...
                <div>
                  <h3 className="font-heading text-lg text-white tracking-wide">{p.titulo}</h3>
                  <div className="w-8 h-0.5 bg-gold-500 mt-1.5 group-hover:w-16 transition-all duration-300" />
                </div>
              </div>

              {/* Description */}
              <p className="text-gray-400 text-sm leading-relaxed mb-4">{p.descricao}</p>
        ...
        <div className="mt-16 text-center">
          <div className="inline-block border border-gold-500/30 bg-gold-500/5 px-8 py-6">
            <p className="text-gray-300 text-sm mb-4">Baixe o programa completo de governo da chapa</p>
